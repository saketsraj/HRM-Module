(function () {
  ...
  function cardHTML(rec) {
    ...
    const link = rec.link
      ? `<a class="link" href="${rec.link}" target="_blank" rel="noopener">Open</a>`
      : "";
    return `
      <div class="item" data-id="${rec.id}">
        <div>
          <h4>${rec.title || "(Untitled)"}</h4>
          <div class="meta">Effective: ${fmtDate(
            rec.date
          )} • Updated: ${fmtDate(rec.updated)}</div>
  ...
  function render() {
    ...
    function matches(rec) {
      if (!q) return true;
      const hay = [rec.title, rec.desc, ...(rec.tags || [])]
        .join(" ")
        .toLowerCase();
    ...
    if (data.benefits.length === 0 && data.policies.length === 0) {
      ...
      data.benefits = [
        ...
        },
        {
          ...
          desc: "₹1,000/month fitness stipend.",
          tags: ["wellness"],
          link: "",
          updated: now,
        },
      ...
      data.policies = [
        {
          ...
          desc: "Professional behavior and anti-harassment policy.",
          tags: ["compliance"],
          link: "",
          updated: now,
        },
        {
          ...
          desc: "Guidelines for company devices and data security.",
          tags: ["security", "it"],
          link: "",
          updated: now,
        },
